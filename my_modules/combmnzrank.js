...
var combMNZ = function(goog, blek, bing, cbToMeta) {
  ...
  //set lengths
  var blekLen = blek.length;
  var bingLen = bing.length;
  var googLen = goog.length;
    ...
    function(callback){//ASSIGN GOOGLE combScoreS
      if(googLen > 0){
        for(var i = 0; i < googLen; i++)
            goog[i].combScore = (1 - (((i+1) - 1)/(googLen)));
      }
      callback(null, goog);
    },
    function(callback){//ASSIGN BING combScoreS
      if(bingLen > 0){
        for(var i = 0; i < bingLen; i++)
            bing[i].combScore = (1 - (((i+1) - 1)/(bingLen)));
      }
      callback(null, bing);
    },
    function(callback){//ASSIGN BLEKKO combScoreS
      ...
      if(blekLen > 0){
        for(var i = 0; i < blekLen; i++)
            blek[i].combScore = (1 - (((i+1) - 1)/(blekLen)));
      }
      callback(null, blek);
    },
    function(callback) {
        ...
        if(googLen > 0){
          if(blekLen > 0 && bingLen > 0)
            combination.items = goog.concat(bing.concat(blek));
          else if(bingLen > 0)
            combination.items = goog.concat(bing);
          else if(blekLen > 0)
            combination.items = goog.concat(blek);
        }
        else{
          if(blekLen > 0 && bingLen > 0)
            combination.items = bing.concat(blek);
          googBool = false
        }
     ...
     async.map(combPreSortAggregList.items,
      function(item, cbAddScore) {
        ...
        if(item.hasOwnProperty('googPos')){
           ...
           if(googposition > -1){
              item.combScore += goog[googposition].combScore;
              item.combScore *= 2;
           }
           ...
           if(item.hasOwnProperty('bingPos')){
              ...
              if(bingposition > -1 && googposition > -1){
                 ...
                 item.combScore += bing[bingposition].combScore;
                 item.combScore *= 3;
              }
