...
function cluster (goog, blek, bing, cbToMeta) {

	var blekLen = blek.length;
   var bingLen = bing.length;
   var googLen = goog.length;

   //Set amount of blek objs to cluster
   if(blekLen <= 30 && blekLen > 0){
      //Do nothing
   }
   else if(blekLen != 0){
      blekLen = 30;
   ...
   for(var i = blekLen -1; i >= 0; i--){
     ...
     if(blek[i].hasOwnProperty('tokenArr'))
        clustUniqs[blek[i].url] = blek[i];
   }
   for(var i = bingLen - 1; i >=0; i--){
     clustUniqs[bing[i].Url] = bing[i];
   }
   for(var i = googLen -1 ; i >= 0; i--){
     clustUniqs[goog[i].link] = goog[i];
   }
