      ...
      function(callback) {
          ...
          if(googLen > 0){
            if(blekLen > 0 && bingLen > 0)
               comb.items = goog.concat(bing.concat(blek));
            else if(bingLen > 0)
               comb.items = goog.concat(bing);
            else if(blekLen > 0)
               comb.items = goog.concat(blek);
          }
          else{
            if(blekLen > 0 && bingLen > 0)
               comb.items = bing.concat(blek);
            googBool = false
          }
         ...
         async.map(rrfPreSortAggregList.items,
          function(item, cbAddRrfScore) {
            ...
            if(item.hasOwnProperty('googPos')){
               ...
               if(googposition > -1)
                  item.rrfScore += goog[googposition].rrfScore;

               if(item.hasOwnProperty('bingPos')){
                  bingposition = item.bingPos;
                  if(bingposition > -1)
                     item.rrfScore += bing[bingposition].rrfScore;
               }
            }
